...
export default function CompletedBookings() {
  ...
  const fetchBookings = async () => {
    try {
      setError(null);
      const response = await bookingService.listBookings({ status: 'completed' });
      setBookings(response.results);
    } catch (err: any) {
      console.error('Fetch completed bookings error:', err);
  ...
  if (bookings.length === 0) {
    return (
      <View style={styles.centerContainer}>
        <ThemedText style={{ color: secondaryTextColor }}>No completed bookings</ThemedText>
      </View>
    );
  }
